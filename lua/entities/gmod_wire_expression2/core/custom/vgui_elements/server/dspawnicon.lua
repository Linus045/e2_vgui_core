...
__e2setcost(5)
local function isValidDSpawnIcon(panel)
...
registerType("dspawnicon", "xdi", nil,
    ...
    function(v)
        return not isValidDSpawnIcon(v)
    end
)
...
e2function number operator_is(xdi pnldata)
    return isValidDSpawnIcon(pnldata) and  1 or 0
end

-- if (!B)
e2function number operator!(xdi pnldata)
    return isValidDSpawnIcon(pnldata) and  0 or 1
end
...
e2function number operator==(xdi ldata, xdi rdata)
    if not isValidDSpawnIcon(ldata) then return 0 end
    if not isValidDSpawnIcon(rdata) then return 0 end
...
--TODO: Check if the entire pnl data is equal (each attribute of the panel)
e2function number operator!=(xdi ldata, xdi rdata)
    if not isValidDSpawnIcon(ldata) then return 1 end
    if not isValidDSpawnIcon(rdata) then return 1 end
